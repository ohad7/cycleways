...
assert.match(reviewScript, /Replace published proof/);
assert.match(reviewScript, /startJob\(step\.job, \{ force: true \}\)/);
assert.match(reviewScript, /previously published proof/);
assert.match(reviewScript, /function reviewStageAttempt/);
assert.match(reviewScript, /\["capture", "render"\]\.includes\(step\.id\)/);
assert.match(reviewScript, /activateWorkflowStep\(step, \{ fromStageCard: true \}\)/);
assert.match(reviewScript, /Use the main workflow action to create a new one/);
