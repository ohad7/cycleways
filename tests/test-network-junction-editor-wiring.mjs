...
assert.match(server, /junctionAttachmentsApplied/);
assert.match(client, /data-junction-delete-draft/);
assert.match(client, /deleteSelectedJunctionDraft/);
...
assert.match(server, /body\?\.action === "delete"/);
assert.match(server, /Published junctions cannot be deleted/);
assert.match(server, /url\.pathname === "\/api\/network-junctions\/review"/);
assert.match(client, /data-movement-review="unavailable"/);
assert.match(client, /data-junction-crossing/);
assert.match(client, /createCrossingFromSelectedJunctionMovement/);
