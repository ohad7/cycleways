...
>
> **Polish pass (later same day), commits `86fcf83`→`da7f808`:**
> - Real **photo thumbnails** on the Discover cards — fixed by authoring the
>   image `require()` map INSIDE the app (`apps/mobile/src/planner/routeImages.js`,
>   generated by the sync script) instead of in `@cycleways/core` (Metro won't
>   serve images required from the symlinked core package; the webp→png
>   conversion stays). Dead `getImageAsset` + core `IMAGE_ASSETS` removed.
> - Circular tonal Build tool buttons + brand-forest primary.
> - **Map controls pinned to the top-right** (`insets.top + 10`), with the search
>   pill shortened (`right: 66`) to make room — they are NOT a sheet-following
>   cluster (that approach was tried in `86fcf83` and replaced).
> - **Full-bleed map** (`da7f808`): removed the `SafeAreaView` wrapper in
>   `App.js` that was double-offsetting content (big empty band above the search
>   box); the map now runs under the status bar and overlays apply their own
>   safe-area insets. **If you add native nav chrome, do NOT reintroduce a
>   SafeAreaView wrapper around the map — use `useSafeAreaInsets` in the overlay.**
>
> **Deferred:** **Build-panel file extraction** to `planner/BuildPanel.jsx` —
> the Build panel already reads as branded-native (eyebrow/title/Ionicons tools/
> cream stat tiles/elevation), so the extraction is organizational-only cleanup,
> not done. (expo-image was tried for thumbnails and backed out — native ABI
> mismatch with expo-modules-core 56.0.14.)
>
