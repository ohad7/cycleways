...
>
> **Polish pass (later same day):** real **photo thumbnails** now render on the
> Discover cards — fixed by authoring the image `require()` map INSIDE the app
> (`apps/mobile/src/planner/routeImages.js`, generated by the sync script)
> instead of in `@cycleways/core` (Metro won't serve images required from the
> symlinked core package; the webp→png conversion stays). The dead
> `getImageAsset` + core `IMAGE_ASSETS` were removed. The floating **map
> controls were moved to the top-right** under the search pill (clustered with
> search/legend per request), replacing the earlier sheet-following behavior.
> Also: circular tonal Build tool buttons + forest primary.
>
