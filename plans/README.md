...
- `turn-by-turn-improvements/` - design (and plan) for the post-ride follow-up to native turn-by-turn: a route-distance segment-span index feeding a network-aware context line, a single smooth directional rider puck, off-route/approach guidance (arrow+distance now, rejoin routing later), and two test harnesses (node GPS-track replay + an in-app simulate-ride dev mode).
