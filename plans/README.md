...
- `route-fit-on-play/` - design for fitting the route to the map on a fresh play, with overlay-aware padding (measured from `data-route-fit-obstruct` elements) so the play controls and other on-map overlays don't hide the route.
