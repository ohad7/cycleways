...
1. **A slim mini-confirm card** replaces the full-screen setup sheet as the
   ...
   behind a secondary `הגדרות רכיבה` screen.
...
- The pending ride plan (route token/slug, direction, selected start progress)
  persists across the handoff, including app kill. On return, validate the
  plan against the reloaded route, obtain a fresh fix, and resume the same
  approach state — or route guidance if the rider is already at the start.
...
- **Location permission denied:** card explains, allows manual start via
  `הגדרות רכיבה`, retry, or starting anyway (approach state then shows the
  route and start marker without a distance until a fix arrives).
